 ...
 * gradient in cover.ts's makeScarfCoverPng for the claim-mint path).
 *
 * VENDORED verbatim (typed) from design/scarf-artwork/scarf-svg.mjs — the design lane owns the
 * source; this copy lives in-lane so it ships in the stands Docker image (which copies
 * services/stands + contracts only, never design/). Re-vendor if the design updates.
 *
 * One pure function, zero runtime deps: fixture facts in → a self-contained SVG string out
 * (fonts inlined as data-URI @font-face; degrades to the Arial Black stack where a viewer refuses
 * fonts in <img>-loaded SVGs — the v1 look, not a break). The cloth is EVENT-WOVEN: goals land as
 * gold-ringed medallions at their minute, cards as satin ticks at the top selvage in their real
 * colour. Empty events → plain cloth, still honest. Never a mark for an event that didn't happen.
 *
 *   scarfSvg({home:{tri,color}, away:{tri,color}, score:{h,a}, dateISO, serial,
 *             events:[{kind:'goal'|'yellow'|'red', minute, side:'home'|'away'}]})
 *   → Buffer.from(svg) uploaded as image/svg+xml where makeScarfCoverPng was used.
 */
import { ANYBODY9, YOUNGSERIF } from './scarf-fonts';

export interface ScarfSvgEvent {
  kind: 'goal' | 'yellow' | 'red';
  minute: number;
  side: 'home' | 'away';
}
export interface ScarfSvgInput {
  home: { tri: string; color: string };
  away: { tri: string; color: string };
  score: { h: number; a: number };
  dateISO?: string;
  serial?: string;
  events?: ScarfSvgEvent[];
}
...
function ink(hex: string, floor = 0.55): string {
  const h = String(hex || '#8C8467').replace('#', '');
  const [r, g, b] = [0, 2, 4].map((i) => parseInt(h.slice(i, i + 2) || '8c', 16)) as [number, number, number];
  const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
  if (lum <= floor) return '#' + h;
  const k = floor / lum;
  const c = (n: number) => Math.round(n * k).toString(16).padStart(2, '0');
  return '#' + c(r) + c(g) + c(b);
}

export function scarfSvg(o: ScarfSvgInput): string {
  const W = 1024, H = 1024;
  ...
  const fringe = (x0: number, dir: number): string => {
    let s = '';
    for (let i = 0; i < 14; i++) {
  ...
  const CLEAR = 30;
  const minuteX = (min: number): number => {

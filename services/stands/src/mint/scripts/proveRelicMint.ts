/**
 * END-TO-END DEVNET RELIC-MINT PROOF — run with:
 *   cd services/stands && npx tsx src/mint/scripts/proveRelicMint.ts
 *
 * Modeled on STRATA's `src/mint/scripts/proveDevnet.ts`. Exercises the exact Phase-2 (Irys/Arweave)
 * + Phase-3 (mpl-core) core the app would use to mint a MatchRelic, driven by a gitignored devnet
 * keypair so it runs headless. It:
 *   1. loads/creates a SEPARATE gitignored devnet mint key (never the production relayer key),
 ...
 *   6. prints the asset + signature + explorer links and writes MINT-DEVNET-PROOF.md.
 *
 * Security: DEVNET ONLY. Only ever touches devnet and a local gitignored key. No secret bytes are
 * ever logged; RPC api-keys are redacted. The mainnet path is never built.
 */
...
async function main(): Promise<void> {
  ...
  const uris = await uploadRelic({ bytes: cover, mime: 'image/png' }, relic, umi, {
    ...
    metaTransform: (md) => ({
      ...md,
      description: `[ROOOT relic devnet mint-pipeline PROOF — synthetic sample data, not a live capture.] ${md.description}`,
    }),
  });
  ...
  // Mint on devnet.
  console.log('• minting Metaplex Core relic on devnet…');
  const result = await mintRelic(relic, uris, umi, 'devnet');

  console.log('\n✓ RELIC MINTED ON DEVNET');
  ...
  const doc = `# ROOOT — devnet relic-mint proof

Generated by \`services/stands/src/mint/scripts/proveRelicMint.ts\` at ${stamp}.
...
`;
  fs.writeFileSync(path.resolve('MINT-DEVNET-PROOF.md'), doc);
  console.log('• wrote services/stands/MINT-DEVNET-PROOF.md');

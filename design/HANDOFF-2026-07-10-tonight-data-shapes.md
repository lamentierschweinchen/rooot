...
moment of the call (the courage weight: calling a side at 16% is not calling it at 60%). The call
resolves on the next REAL goal (side match → correct, everything else → wrong) or at FULL_TIME for a
'none' call ('none' → correct, any side call → wrong). One open call per fan at a time — a new call
REPLACES the old one until resolution; the book empties the instant a cycle resolves, and a fan can
call again for the next goal. **Server mechanism is built and dev-verified tonight; this is the
seam for your build, not live in production yet — check with the coordinator before wiring a real
surface to it.**
...
{ type:'nextGoalVerdict', matchId, anonId,
  ...
  happened: 'home'|'away'|'none',              // what actually happened (the scoring side, or 'none' at FT)
...
- **Always show `open.n`** next to any home/away/none split — "n:3", never an unlabeled 60/33/…
  ...
  `marketAtTs`/`marketAtCall` are `null` until the first odds tick lands for that match — render an
  honest "waiting for the wire" state, never a fabricated 33/33/33.
...
- **Calls are binary outcomes, never a blended percentage.** `outcome` is 'correct'/'wrong' —
  ...
  split (`open`) is a raw count of real fans with an open call, never converted into or displayed as
  a probability.
- **Only live play accepts calls** (FIRST_HALF/SECOND_HALF/EXTRA_TIME) — a call sent pre-kickoff,
  ...
  the fan). Design should gate the call UI the same way: hide or disable it outside live play rather
  than let a fan tap into a call that quietly goes nowhere.

Server-side dev-verified (`services/stands/src/dev/next-goal-check.ts`, `npm run check:next-goal`,
25/25 assertions): call-replace semantics, the pre-kickoff silent-drop, a real goal resolving the
book with correct/wrong verdicts and market stamps, re-emission dedup (a goal's Confirmed
upgrade can't double-resolve), 'none' resolving correct at FULL_TIME, fanStats accumulating at
resolution (not at call time — a replaced call is never double-counted), and a real process restart
mid-cycle restoring the open call + counters. Full existing suite re-run green, zero regressions.
